...
| gRPC | Unary and server streaming complete | Native Protobuf gRPC over negotiated HTTP/2, five-byte incremental framing, backpressured server messages, completion trailers, deadlines, ASCII/binary metadata, structured status/message/details, strict size/count/media validation, real h2c integration; incremental client/bidirectional upload streaming and message compression remain pending |
