...
| Area | Status | Notes |
| --- | --- | --- |
| Runtime and HTTP transport position | Complete | The default HTTP/1.1 transport preserves `std/asyncdispatch` Future/await interoperability and builds on `std/httpclient`; Joubako does not own a private event loop or HTTP/1.1 parser. The explicitly selected HTTP/2 transport uses the declared libcurl Nim binding plus an HTTP/2-capable system libcurl while preserving the same public Future and Result contract. |
| Common request/response/error contracts | Complete | `Future[JResult[T]]`, bounded HTTP error response snapshots without request secrets, attempt counts, cancellation, deadlines, limits, synchronous and backpressured asynchronous progress consumers, idempotent Result-valued client shutdown through composable transports |
| FlowBrigade resilience | Complete | Async retry, circuit breaker, token-bucket rate limit, bulkhead |
| HTTP(S) and JSON | Complete | Verified TLS chain and hostname/IP by default, custom CA/mTLS/cipher configuration, HTTP/SOCKS proxy selection with environment and NO_PROXY rules, keep-alive reuse keyed by origin and proxy, bounded idle pool, bounded gzip/deflate decoding, bounded opt-in cookie jar across redirects, streaming limit checks, validated Range download resume, redirects, typed JSON |
...
| NIF/BIF through NIFKit | Complete for NIFKit v0.4.0 | Raw NIF text and typed Nim-value request APIs over BIF v5 and typed data profile v2, bounded `NifBytes`, direct typed BIF conversion without intermediate NIF text, Joubako-owned finite network defaults for all codec resources, named method/exact-path/longest-prefix policies, typed streaming multipart with separate metadata/file/total budgets and HTTP/2 runtime file accounting, independently configurable encode/decode limits, strict typed compatibility defaults, and structured code/offset/path errors |
...
| Undefined behavior, leaks, and races | Complete | UBSan and four-thread TSan codec probes on Linux/macOS; standalone LSan plus integrated ASan leak detection and Valgrind on Linux; ARC and ORC are covered for every maintained combination; macOS/Windows do not claim leak detection and retain full tests, type checks, and ASan memory-safety coverage |
