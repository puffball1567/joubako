...
| NIF/BIF through NIFKit | Complete for NIFKit v0.3.1 | Raw NIF text and typed Nim-value request APIs over BIF v5, direct typed BIF conversion without intermediate NIF text, Joubako-owned finite network defaults for all codec resources, named method/exact-path/longest-prefix policies, typed streaming multipart with separate metadata/file/total budgets and HTTP/2 runtime file accounting, independently configurable encode/decode limits, strict typed compatibility defaults, and structured code/offset/path errors |
